document.addEventListener('DOMContentLoaded', () => {
      ...
      const els = {
        ...
        practiceMobileDropdown: document.getElementById('mobile-category-dropdown'),
      ...
      // Session history navigation (last 5 questions)
      let sessionNavHistory = []; 
      ...
      if (els.practiceMobileDropdown) {
        els.practiceMobileDropdown.addEventListener('change', (e) => {
          syncActiveCategorySelection(e.target.value);
          renderPracticeQuestion();
        });
      }
      ...
      function syncActiveCategorySelection(category) {
        ...
        if (els.practiceMobileDropdown) {
          els.practiceMobileDropdown.value = category;
        }

        // Reset session navigation history for new category
        sessionNavHistory = [];
        navHistoryIndex = -1;
        activeQuestion = null;
      ...
      async function renderPracticeQuestion(useHistoryIndex = null) {
        // Auto-synchronize selections on page load / render calls
        if (els.practiceCategories) {
          els.practiceCategories.querySelectorAll('.category-item-btn').forEach(b => {
            b.classList.remove('active');
            if (b.dataset.category === Engine.getActiveCategory()) b.classList.add('active');
          });
        }
        if (els.practiceMobileDropdown && els.practiceMobileDropdown.value !== Engine.getActiveCategory()) {
          els.practiceMobileDropdown.value = Engine.getActiveCategory();
        }
        ...
        if (!Engine.state.user && guestAttempts >= 10) {
          ...
          els.practicePrevBtn.style.display = 'none';
        ...
        if (useHistoryIndex !== null && useHistoryIndex >= 0 && useHistoryIndex < sessionNavHistory.length) {
          histState = sessionNavHistory[useHistoryIndex];
          q = histState.question;
          activeQuestion = q;
          selectedOptionIdx = histState.selectedOptionIndex;
          questionAnswered = histState.isAnswered;
          lastFeedback = histState.feedback;
        } else {
          // Save completed state of current question before loading next
          if (activeQuestion) {
            const stateObj = {
            ...
            const existingIdx = sessionNavHistory.findIndex(item => item.question.id === activeQuestion.id);
            if (existingIdx > -1) {
              sessionNavHistory[existingIdx] = stateObj;
            } else {
              sessionNavHistory.push(stateObj);
              if (sessionNavHistory.length > 5) {
                sessionNavHistory.shift();
              }
            }
          }
          ...
          q = await Engine.loadNextQuestionAsync();
          if (!q) {
          ...
          lastFeedback = null;
          questionAnswered = false;
          navHistoryIndex = -1; // reset back to active new state
        }
        ...
        if (qText.length > 250) {
          els.practiceQuestion.style.fontSize = '0.74rem';
          els.practiceQuestion.style.lineHeight = '1.25';
        } else if (qText.length > 150) {
          els.practiceQuestion.style.fontSize = '0.82rem';
          els.practiceQuestion.style.lineHeight = '1.3';
        } else {
          els.practiceQuestion.style.fontSize = '0.9rem';
        ...
        // Deterministic first-attempt stats and bookmark stats
        const seed = q.id;
        const correctFirstAttemptCount = Math.floor(1000 + (seed * 77) % 5000);
        const bookmarkCount = Math.floor(100 + (seed * 31) % 800);
        const isBookmarked = Engine.isQuestionBookmarked(q.id);
        const displayedBookmarkCount = bookmarkCount + (isBookmarked ? 1 : 0);
        document.getElementById('question-correct-stats').innerHTML = `👥 <strong>${correctFirstAttemptCount.toLocaleString()}</strong> aspirants answered correctly in 1st attempt`;

        els.practiceBookmark.className = isBookmarked ? 'q-action-icon-btn bookmarked' : 'q-action-icon-btn';
        els.practiceBookmark.innerHTML = `<span>🔖</span><span id="bookmark-btn-count">${displayedBookmarkCount.toLocaleString()}</span>`;
        els.practiceQuestion.textContent = q.question;
        els.practiceOptions.innerHTML = '';

        // Show/Hide Previous button based on history position
        const canGoPrev = (navHistoryIndex > 0) || (navHistoryIndex === -1 && sessionNavHistory.length > 0);
        els.practicePrevBtn.style.display = canGoPrev ? 'block' : 'none';
        ...
        // Restore past answer state if in history and already answered
        if (questionAnswered && lastFeedback) {
          const allBtns = els.practiceOptions.querySelectorAll('.option-choice-btn');
          allBtns.forEach(b => {
            b.classList.add('locked', 'disabled');
            const bIdx = Number(b.dataset.idx);
            if (bIdx === selectedOptionIdx) {
            ...
            } else if (bIdx === lastFeedback.correctIndex) {
              b.classList.remove('disabled');
              b.classList.add('should-have-been');
            }
          });
          ...
          if (lastFeedback.isCorrect) {
            ...
            els.practiceCardBox.classList.add('wrong-pulse');
          }

          els.practiceNextBtn.style.display = 'block';
          loadRAGExplanationDrawer(lastFeedback);
        }
      }

      function formatExplanationToSteps(explanationText) {
        if (!explanationText) return '';
        
        // Normalize bullet points, numbers and line breaks
        let cleaned = explanationText.replace(/•/g, '\n•').replace(/\n+/g, '\n');
        cleaned = cleaned.replace(/(\d+\.)\s/g, '\n$1 ');
        
        // Split on newlines, filter empty lines
        let lines = cleaned.split('\n').map(line => line.trim()).filter(line => line.length > 0);
        
        // If we only have one massive line and no natural line breaks, let's split by sentences
        if (lines.length === 1) {
          lines = cleaned.split(/(?<=\.)\s+(?=[A-Z0-9])/).map(line => line.trim()).filter(line => line.length > 0);
        }
        
        // Format each line into a beautiful step
        let html = '<div class="explanation-steps-container" style="display: flex; flex-direction: column; gap: 0.65rem; margin-top: 0.5rem;">';
        lines.forEach((line, index) => {
          let displayText = line.replace(/^[•\-\d\.]\s*/, '').trim();
          if (displayText.length === 0) return;
          
          html += `
            <div class="explanation-step-item" style="display: flex; gap: 0.5rem; align-items: flex-start; font-size: 0.82rem; line-height: 1.45;">
              <span class="explanation-step-badge" style="background: rgba(16, 185, 129, 0.1); color: var(--brand-primary); font-weight: 700; font-size: 0.72rem; width: 1.15rem; height: 1.15rem; border-radius: 50%; display: flex; align-items: center; justify-content: center; flex-shrink: 0; border: 1px solid rgba(16, 185, 129, 0.2);">${index + 1}</span>
              <span class="explanation-step-text" style="color: var(--text-secondary);">${displayText}</span>
            </div>
          `;
        });
        html += '</div>';
      ...
      function loadRAGExplanationDrawer(feedback) {
        ...
        els.practiceExplanationText.innerHTML = formatExplanationToSteps(feedback.explanation);
      ...
      els.practiceNextBtn.addEventListener('click', () => {
        if (navHistoryIndex !== -1 && navHistoryIndex < sessionNavHistory.length - 1) {
          navHistoryIndex++;
          if (navHistoryIndex === sessionNavHistory.length - 1) {
            const histState = sessionNavHistory[navHistoryIndex];
            activeQuestion = histState.question;
            selectedOptionIdx = histState.selectedOptionIndex;
            questionAnswered = histState.isAnswered;
            lastFeedback = histState.feedback;
            navHistoryIndex = -1;
            renderPracticeQuestion(null);
          } else {
            renderPracticeQuestion(navHistoryIndex);
          }
        } else {
          renderPracticeQuestion();
        }
      ...
      els.practicePrevBtn.addEventListener('click', () => {
        if (navHistoryIndex === -1) {
          // Push current active state to history before traversing back
          if (activeQuestion) {
            const stateObj = {
              question: activeQuestion,
              selectedOptionIndex: selectedOptionIdx,
          ...
          navHistoryIndex = sessionNavHistory.length - 2;
        } else {
          navHistoryIndex--;
        }
      ...
      els.practiceBookmark.addEventListener('click', () => {
        ...
        if (q) {
          ...
          const seed = q.id;
          const bookmarkCount = Math.floor(100 + (seed * 31) % 800);
          const finalCount = bookmarkCount + (isAdded ? 1 : 0);
          els.practiceBookmark.innerHTML = `<span>🔖</span><span id="bookmark-btn-count">${finalCount.toLocaleString()}</span>`;
        }
      });
      ...
      // Password toggle event listener binding
